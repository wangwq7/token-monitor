...
const LIMIT_CAPABILITY_TAG_KEYS = {
  ...
  'Agent Plan': 'settings.limits.capability.agentPlan',
...
const HOME_MODULE_OPTIONS = [
  ...
  { id: 'live', labelKey: 'home.live', viewId: 'live' }
];
...
// Linear blend of two #rrggbb colors; t in [0,1]. Falls back to `a` on bad input.
function blendHex(a, b, t) {
  const ma = String(a || '').replace('#', '');
  const mb = String(b || '').replace('#', '');
  if (!/^[0-9a-f]{6}$/i.test(ma) || !/^[0-9a-f]{6}$/i.test(mb)) return a || '#6ab4f0';
  const ar = parseInt(ma.slice(0, 2), 16), ag = parseInt(ma.slice(2, 4), 16), ab = parseInt(ma.slice(4, 6), 16);
  const br = parseInt(mb.slice(0, 2), 16), bg = parseInt(mb.slice(2, 4), 16), bb = parseInt(mb.slice(4, 6), 16);
  const k = Math.max(0, Math.min(1, Number(t) || 0));
  const ch = (from, to) => Math.round(from + (to - from) * k).toString(16).padStart(2, '0');
  return `#${ch(ar, br)}${ch(ag, bg)}${ch(ab, bb)}`;
}

// Status gradient for limit rings: keep the provider brand color while quota is
// healthy (>=50% left), then blend toward amber/orange/red as remaining drops so
// a near-spent window reads as a warning at a glance. remainingPercent is the
// "left" percent (0-100). Used for the Home limit ring arc (both percent and
// balance windows - balance carries remainingPercent = amount/(amount+spend)).
function limitStatusColor(remainingPercent, baseHex) {
  const remaining = Math.max(0, Math.min(100, Number(remainingPercent)));
  if (Number.isNaN(remaining)) return baseHex || '#6ab4f0';
  const base = baseHex || '#6ab4f0';
  if (remaining >= 50) return base;
  if (remaining >= 25) return blendHex(base, '#f0a830', (50 - remaining) / 25); // -> amber
  if (remaining >= 10) return blendHex('#f0a830', '#e8743b', (25 - remaining) / 15); // amber -> orange
  return blendHex('#e8743b', '#e5484d', (10 - remaining) / 10); // orange -> red
}
...
function availableBreakdownIds() {
  const order = ['home', baseBreakdownOrder[0], 'status', 'live', 'trends', ...baseBreakdownOrder.slice(1)];
...
// Live view: surfaces the session(s) actively burning tokens right now and the
// model(s) driving that spend. "Active" = lastUsedAt within an adaptive window
// (the jsonl transcript's last-line timestamp, which moves as the session writes).
// Data comes straight from state.stats.periods.today.sessions (each session carries
// a per-model token map), so this view updates on every stats:push like the totals.
const LIVE_ACTIVE_WINDOW_MS = 60_000;
const LIVE_RECENT_LIMIT = 8;
const LIVE_SESSIONS_WITH_DETAIL = new Set(['claude', 'codex', 'opencode']);

function liveActiveWindowMs() {
  const refresh = Number(state.settings?.refreshMs) || 15000;
  return Math.max(LIVE_ACTIVE_WINDOW_MS, refresh * 2);
}

function liveSessionModelRows(session) {
...
function liveSessionsForPeriod(period, nowMs) {
  const entries = Object.entries(period?.sessions || {})
    .map(([key, session]) => {
      const lastMs = Date.parse(session?.lastUsedAt || session?.startedAt || '');
      return {
        key,
    ...
    .filter((session) => session.totalTokens > 0 && session.lastMs > 0)
    .sort((a, b) => b.lastMs - a.lastMs || b.totalTokens - a.totalTokens);
  const threshold = nowMs - liveActiveWindowMs();
  const active = entries.filter((session) => session.lastMs >= threshold);
  const recent = entries.filter((session) => session.lastMs < threshold).slice(0, LIVE_RECENT_LIMIT);
  return { active, recent };
}

function liveModelBreakdownNode(session, { total }) {
  const models = liveSessionModelRows(session);
  const wrap = document.createElement('div');
  wrap.className = 'live-model-breakdown';
  if (!models.length) {
    const empty = document.createElement('div');
    empty.className = 'live-model-empty';
    empty.textContent = t('live.noModels');
    wrap.append(empty);
    return wrap;
  }
  const ref = Math.max(1, total || session.totalTokens || 0);
  for (const { model, tokens } of models) {
    ...
    const value = document.createElement('span');
    value.className = 'live-model-value';
    value.textContent = formatNumber(tokens);
    const bar = document.createElement('div');
    bar.className = 'live-model-bar';
    const fill = document.createElement('div');
...
function liveHeroCard(session, nowMs) {
  const card = document.createElement('div');
  card.className = 'live-hero-card';
  const clientLabel = clientLabels[session.client] || session.client || 'Session';
  const models = liveSessionModelRows(session);
  const primaryModel = models[0]?.model || '';
  const head = document.createElement('div');
  head.className = 'live-hero-head';
  const iconKind = iconKindFor({ client: session.client }, 'session');
  const mark = document.createElement('span');
  mark.className = iconKind.kind === 'icon' ? `live-hero-mark row-icon ${iconKind.iconClass}` : 'live-hero-mark dot';
  if (iconKind.kind === 'dot') mark.style.background = clientColors[session.client] || clientColors.default || 'var(--accent)';
  const titleWrap = document.createElement('div');
  titleWrap.className = 'live-hero-title-wrap';
  const title = document.createElement('div');
  title.className = 'live-hero-title';
  const nameSpan = document.createElement('span');
  nameSpan.textContent = clientLabel;
  if (primaryModel) {
    const sep = document.createElement('span');
    sep.className = 'live-hero-sep';
    sep.textContent = ' · ';
    const modelSpan = document.createElement('span');
    modelSpan.className = 'live-hero-model';
    modelSpan.textContent = models.length > 1 ? t('live.modelsCount', { count: models.length }) : primaryModel;
    title.append(nameSpan, sep, modelSpan);
  } else {
    title.append(nameSpan);
  }
  const meta = document.createElement('div');
  meta.className = 'live-hero-meta';
  const pulse = document.createElement('span');
  pulse.className = 'live-pulse';
  const pulseDot = document.createElement('span');
  pulseDot.className = 'live-pulse-dot';
  pulse.append(pulseDot, document.createTextNode(t('live.active')));
  const time = document.createElement('span');
  time.className = 'live-hero-time';
  time.textContent = sessionRowsApi.compactSessionTime(session.lastUsedAt, new Date(nowMs));
  meta.append(pulse, time);
  titleWrap.append(title, meta);
  const metrics = document.createElement('div');
  metrics.className = 'live-hero-metrics';
  const value = document.createElement('div');
  value.className = 'live-hero-value';
  value.textContent = formatNumber(session.totalTokens);
  const cost = document.createElement('div');
  cost.className = 'live-hero-cost';
  cost.textContent = formatCost(session.costUsd);
  metrics.append(value, cost);
  head.append(mark, titleWrap, metrics);
  card.append(head, liveModelBreakdownNode(session, { total: session.totalTokens }));
  if (LIVE_SESSIONS_WITH_DETAIL.has(session.client)) {
    card.classList.add('live-hero-clickable');
    card.setAttribute('role', 'button');
    card.setAttribute('tabindex', '0');
    card.title = t('live.openDetail');
    const client = session.client;
    const sessionId = session.sessionId;
    const sessionCost = session.costUsd;
    const open = () => openSessionDetail({ client, sessionId, sessionCost, title: `${clientLabel} session ${sessionId}` });
...
function liveRecentRow(session, nowMs) {
  const row = document.createElement('div');
  row.className = 'live-recent-row';
  const clientLabel = clientLabels[session.client] || session.client || 'Session';
  const models = liveSessionModelRows(session);
  const primaryModel = models[0]?.model || '';
  const head = document.createElement('div');
  head.className = 'live-recent-head';
  const iconKind = iconKindFor({ client: session.client }, 'session');
  const mark = document.createElement('span');
  mark.className = iconKind.kind === 'icon' ? `live-recent-mark row-icon ${iconKind.iconClass}` : 'live-recent-mark dot';
  if (iconKind.kind === 'dot') mark.style.background = clientColors[session.client] || clientColors.default || 'var(--accent)';
  const titleWrap = document.createElement('div');
  titleWrap.className = 'live-recent-title-wrap';
  const title = document.createElement('div');
  title.className = 'live-recent-title';
  title.textContent = primaryModel ? `${clientLabel} · ${models.length > 1 ? t('live.modelsCount', { count: models.length }) : primaryModel}` : clientLabel;
  const sub = document.createElement('div');
  sub.className = 'live-recent-sub';
  const time = sessionRowsApi.compactSessionTime(session.lastUsedAt, new Date(nowMs));
  const msg = session.messageCount > 0 ? `${formatNumber(session.messageCount)} ${t('live.messages')}` : '';
  sub.textContent = [time, msg].filter(Boolean).join(' · ');
  titleWrap.append(title, sub);
  const metrics = document.createElement('div');
  metrics.className = 'live-recent-metrics';
  const value = document.createElement('span');
  value.className = 'live-recent-value';
  value.textContent = formatNumber(session.totalTokens);
  const cost = document.createElement('span');
  cost.className = 'live-recent-cost';
  cost.textContent = formatCost(session.costUsd);
  metrics.append(value, cost);
  head.append(mark, titleWrap, metrics);
  row.append(head);
  // Inline model breakdown (no separate expand state - cheap to always render).
  // Unconditional like the active hero card - a single-model session still shows
  // its bar + token count, otherwise the recent row loses the bar that the hero
  // always renders, so single-model sessions looked incomplete.
  row.append(liveModelBreakdownNode(session, { total: session.totalTokens }));
  return row;
}

function renderLive(period) {
  if (!els.livePanel) return;
  const nowMs = Date.now();
  const { active, recent } = liveSessionsForPeriod(period, nowMs);
  const panel = els.livePanel;
  panel.replaceChildren();
  if (active.length === 0 && recent.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'live-empty';
    const icon = document.createElement('div');
    icon.className = 'live-empty-icon view-icon-live';
    const text = document.createElement('div');
    text.className = 'live-empty-text';
    text.textContent = t('live.empty');
  ...
  if (active.length > 0) {
    ...
    heroWrap.className = 'live-hero-wrap';
    for (const session of active) heroWrap.append(liveHeroCard(session, nowMs));
    panel.append(heroWrap);
  } else {
    const idle = document.createElement('div');
    idle.className = 'live-idle';
    const pulse = document.createElement('span');
    pulse.className = 'live-pulse idle';
    const dot = document.createElement('span');
    dot.className = 'live-pulse-dot';
    pulse.append(dot, document.createTextNode(t('live.idle')));
    idle.append(pulse);
    panel.append(idle);
  }
  if (recent.length > 0) {
    ...
    heading.className = 'live-recent-heading';
    heading.textContent = t('live.recent');
    section.append(heading);
    for (const session of recent) section.append(liveRecentRow(session, nowMs));
...
function homeLimitWindowLabel(window) {
  ...
  // Only honour a provider-specific label (e.g. Volcengine/Z.ai "5-hour"). A bare
  // kind string here would lose the localized "Session"/"Weekly" wording, so fall
  // through to the i18n key when no real label is set.
  if (label && label !== window?.kind) return label;
  const key = {
...
// Home limit ring: a small SVG ring per window. Grey track = the part that has
// passed; colored arc = remaining (or used, in show-used mode, via the shared
// limitFillPercent so it matches the full Limits panel bar + tray bars). The arc
// color blends the provider brand color toward amber/red as remaining drops so a
// near-spent window reads as a warning. Center shows the percent number + a
// left/used suffix. Used for quota (percent) windows only.
const HOME_LIMIT_RING = { r: 18, cx: 22, cy: 22, stroke: 5 };
HOME_LIMIT_RING.circumference = 2 * Math.PI * HOME_LIMIT_RING.r;

function homeLimitRing(window, color) {
  const showUsed = Boolean(state.settings?.showLimitUsed);
  const remaining = Number(window?.remainingPercent);
  const fillPercent = limitFillPercent(remaining, window?.usedPercent, showUsed);
  const safe = Math.max(0, Math.min(100, Number.isFinite(fillPercent) ? fillPercent : 0));
  const { r, cx, cy, stroke, circumference } = HOME_LIMIT_RING;
  const dashoffset = circumference * (1 - safe / 100);
  // Color keys off remaining (the "left" percent) regardless of show-used mode,
  // so a full-used ring is red whether you read left% or used%.
  const arcColor = limitStatusColor(remaining, color);

  const wrap = document.createElement('div');
  wrap.className = 'home-limit-ring';
  ...
  num.textContent = formatPercent(safe);
  const sub = document.createElement('span');
  sub.className = 'ring-sub';
  sub.textContent = limitModeSuffix(showUsed);
  value.append(num, sub);
  wrap.append(value);

  const label = homeLimitWindowLabel(window);
  wrap.setAttribute('role', 'img');
  wrap.setAttribute('aria-label', `${label}, ${formatPercent(safe)} ${limitModeSuffix(showUsed)}`);
  return wrap;
}
...
function renderHomeLiveModule() {
  // "Currently running" is today-scoped (lastUsedAt within the active window),
  // so read today directly rather than the selected period tab - flipping
  // today/month/allTime on Home leaves this module stable. Refreshes on every
  // stats:push via renderHome, same cadence as the total-token counter.
  const today = state.stats?.periods?.today || { sessions: {} };
  const nowMs = Date.now();
  const { active } = liveSessionsForPeriod(today, nowMs);
  const meta = active.length > 0 ? t('home.liveCount', { count: active.length }) : '';
  const { module, body } = homeModuleShell('live', t('home.live'), 'live', meta);

  if (active.length === 0) {
    const empty = document.createElement('div');
    empty.className = 'home-module-empty home-live-empty';
    const pulse = document.createElement('span');
    pulse.className = 'live-pulse idle';
    const dot = document.createElement('span');
    dot.className = 'live-pulse-dot';
    ...
    text.textContent = t('home.noLive');
    empty.append(pulse, text);
    body.append(empty);
    return module;
  }

  // Count modifier drives the column layout (1 = full width, 2 = one row of
  // two, 3-4 = 2x2). Cap at 4 so the grid stays deterministic; the full Live
  // view still shows every active session. Clicks on a clickable hero card
  // (claude/codex/opencode -> openSessionDetail) are stopped here so they
  // don't also bubble up to the module's setBreakdown('live') handler.
  const countClass = active.length === 1 ? 'home-live-count-1'
    : active.length === 2 ? 'home-live-count-2'
    : 'home-live-count-3-4';
  const grid = document.createElement('div');
  grid.className = `home-live-grid ${countClass}`;
  grid.addEventListener('click', (event) => {
    if (event.target.closest('.live-hero-clickable')) event.stopPropagation();
  });
  for (const session of active.slice(0, 4)) grid.append(liveHeroCard(session, nowMs));
  body.append(grid);
  return module;
}
...
function renderHomeTrendsModule() {
  ...
  // day/month use contribHeatmap (hugs the data - only the period's cells render);
  // allTime keeps rollingYearHeatmap (full-year grid, scrollable). rollingYearHeatmap
  // forces a 12-month grid even for a 7-day slice, so it can't be used for day/month.
  ...
  const activity = state.period === 'allTime'
    ...
    : charts.contribHeatmap(dailyWithHeatIntensity(heatPoints), { cell: activityLayout.cell, gap: activityLayout.gap });
  const rangeLabel = state.period === 'allTime' ? t('trends.range.year')
    : state.period === 'month' ? t('trends.range.month') : t('trends.range.week');
  const { module, body } = homeModuleShell('trends', t('home.activity'), 'trends', rangeLabel);
  // allTime re-pins the newest column on every render/resize; day/month don't scroll.
  if (state.period === 'allTime') state.homeActivityFollowEnd = true;
  ...
  // Trend line: slice the daily points by period. allTime uses ALL daily points (a
  // real multi-point curve), not the 2-point monthly series selectPreviewSeries
  // returned - that drew a flat line. day = last 30 days (not 7), month = current
  // month. patchTodayBar keeps today's point live in day view.
...
function renderHome() {
  ...
  const nodes = moduleIds.map((id) => {
    ...
    if (id === 'live') return renderHomeLiveModule();
...
function render() {
  ...
  renderViewSwitcher();
  if (state.openSession && state.breakdown !== 'session' && state.breakdown !== 'live') { state.openSession = null; els.sessionDetail.classList.add('hidden'); els.sessionDetail.replaceChildren(); els.sessionDetailHead.classList.add('hidden'); els.sessionDetailHead.replaceChildren(); }
  ...
  if (state.breakdown === 'home') {
    ...
    els.livePanel.classList.add('hidden');
    ...
    renderServiceStatus();
  } else if (state.breakdown === 'live' && !state.openSession) {
    els.homePanel.classList.add('hidden');
    els.breakdown.classList.add('hidden');
    els.limitsPanel.classList.add('hidden');
    els.serviceStatusPanel?.classList.add('hidden');
